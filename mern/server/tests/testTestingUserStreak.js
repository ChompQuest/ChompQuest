...
async function testTestingUserStreak() {
  ...
  try {
    ...
    if (!user) {
      console.log("[X] User 'testingUser' not found in database");
      return;
    }
    
    console.log(`[Pass] Found test user: ${user.username}`);
    console.log(`Current game stats:`, user.game_stats);
    console.log(`Nutrition goals:`, user.nutritionGoals);
    
    // Test the nutrition streak logic
    const today = new Date();
    const todayStr = today.toISOString().split('T')[0];
    
    // Add some test meals for today (meeting 100% of goals)
    ...
    const testMeals = [
      {
        userId: user._id,
        name: "Test Breakfast",
        calories: 1200, // 40% of 3000
        protein: 76,    // 40% of 190
        carbs: 116,     // 40% of 290
        fat: 40,        // 40% of 99
        mealType: "breakfast",
      ...
      },
      {
        userId: user._id,
        name: "Test Lunch",
        calories: 900,  // 30% of 3000
        protein: 57,    // 30% of 190
        carbs: 87,      // 30% of 290
        fat: 30,        // 30% of 99
        mealType: "lunch",
      ...
      },
      {
        ...
        protein: 57,    // 30% of 190
        carbs: 87,      // 30% of 290
        fat: 29,        // 29% of 99 (total will be 99)
        mealType: "dinner",
    ...
    await db.collection("water_intake").updateOne(
      {
        userId: user._id,
        date: todayStr
      },
      {
        $set: {
          intake: 2900, // 100% of water goal
          updatedAt: new Date()
        }
      },
      { upsert: true }
    );
    console.log("[Pass] Added water intake for today");
    ...
    console.log("[INFO] Today's water intake:", waterIntake);
    ...
    console.log("[Pass] Testing user streak test completed successfully!");
