...
// Hardcoded food data from AddMeal.tsx
...
async function migrateFoodsToDatabase() {
  ...
  try {
    ...
    console.log('🧹 Clearing existing food items (if any)...');
    await foodItemsCollection.deleteMany({});
    
    console.log('📊 Converting hardcoded foods to database format...');
    const foodItems = Object.entries(HARDCODED_FOODS).map(([name, nutrients]) => ({
      name,
      ...
      createdAt: new Date(),
      updatedAt: new Date(),
      createdBy: null, // System migration
      isMigrated: true // Flag to identify migrated items
    }));
    
    console.log(`📦 Inserting ${foodItems.length} food items into database...`);
    const result = await foodItemsCollection.insertMany(foodItems);
    
    console.log(`✅ Successfully migrated ${result.insertedCount} food items to database!`);
    console.log('📋 Sample migrated items:');
    
    // Show first 5 items as examples
    const sampleItems = await foodItemsCollection.find({}).limit(5).toArray();
    sampleItems.forEach((item, index) => {
      console.log(`   ${index + 1}. ${item.name} - ${item.calories} cal, ${item.protein}g protein`);
    });
    
    console.log('\n🎉 Migration completed successfully!');
    console.log(`📊 Total food items in database: ${await foodItemsCollection.countDocuments()}`);
...
export { migrateFoodsToDatabase, HARDCODED_FOODS };